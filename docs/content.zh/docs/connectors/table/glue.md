...
<table class="table table-bordered">
    ...
    <tr>
      ...
      <td>No</td>
      <td style="word-wrap: break-word;">(none)</td>
      <td>HTTP client options for the underlying AWS SDK client, for example <code>http-client.connection-timeout-ms</code>, <code>http-client.socket-timeout-ms</code> and <code>http-client.apache.max-connections</code>.</td>
    </tr>
    </tbody>
</table>
...
5. **Table Types**: Only tables and views created through Flink (Glue table types `TABLE` and `VIEW`) can be read by this catalog. Tables created by other services with other Glue table types (for example `EXTERNAL_TABLE` tables written by Athena or Hive) are listed but cannot be used from Flink, because they do not carry Flink connector properties.
